<template>
 <div  class="pluginbox">
    <template v-if="show">
    <div class="headerline" v-if="pluginData">
      <div class="title">{{ getPluTitle(pluginData.type) }}</div>
      <div class="no">{{ pluginData.uuid }}</div>
    </div>
    <!--编辑区-->
    <div class="content-box" v-if="pluginData">
        <!--1单行文本输入框-->
            ...
            <div class="style-setting">
              ...
              <div class="style-form">
                ...
                <el-form-item label="水平对齐">
                  <el-input v-model="pluginData.style.marginBottom" placeholder="设置图片与文字对齐" @change="changeStyle(pluginData.style.marginBottom+'px','marginBottom')" type="number" min="-120" max="120"/>
                </el-form-item>
              </div>
            </div>
          </el-form>
        ...
        <div class="textsync" v-if="pluginData.type=='textsync'">
          <el-form  label-width="78px" label-position="right">
            ...
            <el-form-item label="计算公式" v-if="pluginData.datafrom=='formula'">
               <el-input v-model="pluginData.dataid"  placeholder="编辑计算公式" clearable/>
            </el-form-item>
            <el-form-item label="默认值">
               <el-input v-model="pluginData.value" @change="changeSetValue(pluginData.value)" clearable placeholder="未加载默认数据（选填）"/>
            </el-form-item>
          </el-form>
        </div>
    </div>
  ...
  <template v-else>
    <el-empty :image-size="180" description="编辑组件区"/>
  </template>
 </div>
</template>
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        /**************样式 */
        //设置样式
        const changeStyle=(val:any,keys:any)=>{
        ...
        const changeBorder=(type:any)=>{
          ...
          pluginDom.dataset.bordertype=type
        }
        /****赋值组件 */
        //组件赋值
