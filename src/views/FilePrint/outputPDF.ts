 ...
 */
export async function outputPDF({ element, contentWidth = 550, footer, header, filename = "测试A4分页.pdf" }) {
  ...
  // 添加页眉
  async function addHeader(pageNum:any, now:any,header:any, pdf:any, contentWidth:any) {
    const newHeader = header.cloneNode(true);
    newHeader.querySelector('.pdf-header-page').innerText = now;
    newHeader.querySelector('.pdf-header-page-count').innerText = pageNum;
    document.documentElement.append(newHeader);
    const { height: headerHeight, data: headerData, width: hWidth } = await toCanvas(newHeader, contentWidth);
    pdf.addImage(headerData, 'JPEG', 0, 0, contentWidth, headerHeight);
  }

  // 添加页脚
  async function addFooter(pageNum:any, now:any, footer:any, pd:any, contentWidth:any) {
  ...
  function updatePos(eheight:any, top:any,one:any) {
  ...
  for (let i = 0; i < pages.length; ++i) {
    message.success({content:`共${pages.length}页， 生成第${i + 1}页`,key:"makepdf"})
    // 根据分页位置新增图片
    ...
    // 添加页眉
    await addHeader(pages.length, i + 1, header, pdf, A4_WIDTH)
    // 添加页脚
